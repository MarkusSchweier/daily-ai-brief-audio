...
`deploy/eval/eval_core/record.py` -- this module never made an AWS call and its
`EvalRecord`/`CandidateAggregate` dataclass shapes are exactly what ADR-0016 D4
reuses as the in-memory shape for `deploy/eval-harness/`'s per-run directory records
(serialized to `scores.json`/`summary.json` instead of a DynamoDB item). The DynamoDB/
S3-storage framing in the docstring above describes the ORIGINAL, now-retired
`deploy/eval/` backbone (ADR-0013); it is left as historical context, not current
behavior -- the harness's own storage is git-tracked files (see
`deploy/eval-harness/harness/run_store.py`).
"""
