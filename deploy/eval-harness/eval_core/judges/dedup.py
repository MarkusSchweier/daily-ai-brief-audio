...
PORTED, UNCHANGED (ADR-0016 Phase 1, 2026-07-07) from `deploy/eval/eval_core/judges/dedup.py`.
In this harness, `prior_briefs_markdown` is supplied by
`harness.dedup_priors.fetch_recent_prior_briefs_markdown()` (the delivery boundary's
read-only `GET /recent-briefs` route) instead of the original
`brief_history.read_recent_prior_briefs()` S3 helper -- this module itself only ever
consumed a plain list of markdown strings, so it needed no change.
"""
