...
module is hand-duplicated (not a shared package) across three independent deploy units.

Duplicated verbatim in:
  - deploy/managed-agent/pipeline/feedback_token.py   (this copy — imported by audio_email.py)
...
Three independent deploy units (a microVM image, a Lambda layer, and a standalone CDK
app), kept identical by hand — same convention as `MAX_AUDIO_ATTACHMENT_BYTES`'s
duplicated-constant docstring elsewhere in this repo. A cross-copy compatibility test
(a token `generate`d by one copy `validate`s under another) pins the three copies to
one wire format; see the test suite for details. Any change here MUST be applied to
the other two copies too.
