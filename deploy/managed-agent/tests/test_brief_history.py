...
def test_source_usage_file_missing_is_handled_gracefully(briefs_bucket, tmp_path):
    """AC-8a / the developer task's own "missing file handled gracefully" requirement:
    an older run, a run before this feature shipped, or a run whose skill version
    doesn't yet emit source-usage.json, must not raise and must not archive anything."""
    ...
    with _pytest.raises(botocore.exceptions.ClientError):
        briefs_bucket.get_object(
            Bucket=brief_history.BUCKET,
            Key=f"briefs/2026-07-03/{brief_history.SOURCE_USAGE_FILENAME}",
        )
...
def test_source_usage_file_read_failure_is_logged_not_raised(tmp_path, capsys):
    ...
    try:
        ...
        archived = brief_history.archive_source_usage_file(
            "unused-s3-client", "2026-07-03", working_folder=str(tmp_path)
        )
