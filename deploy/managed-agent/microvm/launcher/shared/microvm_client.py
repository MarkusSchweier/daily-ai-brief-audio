...
Ported verbatim from AWS's reference implementation (src/functions/shared/microvm_client.py)
— see shared/constants.py for the porting note.

Calls RunMicrovm through the boto3 ``lambda-microvms`` client. Note: as of this
port, ``lambda-microvms`` is a newer/beta service model that may not ship in
every boto3 release yet — confirm ``pip show boto3`` / the installed botocore's
service model includes ``lambda-microvms`` before relying on this in prod; the
reference implementation vendors specific boto3/botocore wheels for exactly this
reason (see requirements.txt in this directory).
"""
...
class Boto3MicroVmClient:
    """RunMicrovm client backed by the ``lambda-microvms`` boto3 service client."""
    ...
    def launch_microvm(
        ...
        # Map to RunMicrovmRequest fields (lambda-microvms API model).
