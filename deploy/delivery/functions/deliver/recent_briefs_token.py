...
/recent-briefs`: mint (candidate side) / verify (here, the delivery Lambda) a
`<payload_b64url>.<sig_b64url>` HMAC-SHA256 token that expires within minutes.
...
Duplicated verbatim (past this docstring) in:
  - deploy/delivery/functions/deliver/recent_briefs_token.py  (this copy -- the
    VERIFY side, imported by recent_briefs_auth.py / handler.py)
  - deploy/candidates/candidate_sync/recent_briefs_token.py  (the MINT side --
    imported by candidate_sync/trigger.py, which mints a token per triggered
    run and substitutes it into a candidate's task prompt)
