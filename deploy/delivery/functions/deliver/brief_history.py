...
Hand-duplicated (PRD docs/prd/agent-system-redesign.md, ADR-0014 Decision 2a): this
file is now copied verbatim into TWO independent deploy units --
`deploy/managed-agent/pipeline/brief_history.py` (this original, still what the LIVE
production microVM image uses, untouched by this phase) and
`deploy/delivery/functions/deliver/brief_history.py` (this copy, the new decoupled
delivery Lambda's own copy). This repo's established convention for exactly this
situation -- see `feedback_token.py`'s docstring (already hand-duplicated across
THREE independent deploy units) and `deploy/eval/functions/common/review_auth.py`'s
docstring citing that same precedent. `deploy/managed-agent/` and `deploy/delivery/`
are independent Lambda deployment units with no shared package, so this is copied,
not imported; any future change here MUST be applied to both copies by hand.
"""
