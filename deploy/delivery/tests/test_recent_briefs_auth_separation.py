...
bearer token and the recent-briefs SIGNED read token are NOT interchangeable.

This is what preserves FR-1/FR-7 (PRD docs/prd/agent-system-redesign.md) by
construction: a `cloud` candidate given ONLY a token signed with the read
secret must be structurally unable to authenticate to `POST /deliver` (and so
can never trigger a real send/fan-out to subscribers), and conversely the
delivery/send token must not authenticate `GET /recent-briefs` either (it was
never signed with the read secret as a `recent-briefs`-scoped token, so
`recent_briefs_token.verify()` rejects it outright) -- so a leaked/rotated
delivery secret does not also grant read access, keeping the two blast radii
independent. This guarantee is UNCHANGED by the 2026-07-06 signed-token
correction (Decision 2d's "Correction: how the read token actually reaches a
`cloud` candidate", status ACCEPTED) -- the correction only bounds the read
token's LIFETIME, not what it can authenticate.
...
READ_SIGNING_KEY = "read-only-secret-xyz"


def _signed_read_token(signing_key: str = READ_SIGNING_KEY, *, ttl_seconds: int = 900) -> str:
    """Mint a real, validly-signed `recent-briefs`-scope token under
    `signing_key` -- what a genuine `cloud` candidate presents to
    `GET /recent-briefs` under the new scheme, in place of the OLD raw-secret
    static bearer value this test file used before the 2026-07-06 correction."""
...
def test_read_token_does_not_authenticate_post_deliver(monkeypatch):
    _configure_both_secrets(monkeypatch, delivery_secret=DELIVERY_TOKEN, read_secret=READ_SIGNING_KEY)

    event = _event_with_bearer(_signed_read_token(), method="POST", path="/deliver")
...
def test_read_token_does_not_authenticate_get_deliver_poll(monkeypatch):
    ...
    event = _event_with_bearer(
        _signed_read_token(), method="GET", path="/deliver/some-id", path_params={"deliveryId": "some-id"}
    )
    result = handler_module.handler(event, None)
...
def test_read_token_does_authenticate_get_recent_briefs(monkeypatch):
    ...
    event = _event_with_bearer(_signed_read_token(), method="GET", path="/recent-briefs")
    result = handler_module.handler(event, None)
...
def test_recent_briefs_fails_closed_through_dispatch_when_no_secret_configured(monkeypatch):
    ...
    never fall open to an unauthenticated read -- even with an otherwise
    perfectly valid signed token."""
...
def test_recent_briefs_fails_closed_through_dispatch_on_malformed_authorization_header(monkeypatch):
    ...
    event = {
        ...
        "headers": {"Authorization": f"Basic {_signed_read_token()}"},  # wrong scheme, not "Bearer"
...
def test_recent_briefs_fails_closed_through_dispatch_on_expired_token(monkeypatch):
    """An EXPIRED signed read token must 401 through the real dispatch too --
    the core property the 2026-07-06 correction adds: even a token that once
    validly authenticated must stop working once its `exp` passes."""
    _configure_both_secrets(monkeypatch, delivery_secret=DELIVERY_TOKEN, read_secret=READ_SIGNING_KEY)

    expired_token = _signed_read_token(ttl_seconds=-1)
    event = _event_with_bearer(expired_token, method="GET", path="/recent-briefs")
    result = handler_module.handler(event, None)
...
def test_non_get_to_recent_briefs_is_405_not_a_fallthrough_to_delivery(monkeypatch):
    ...
    event = _event_with_bearer(_signed_read_token(), method="POST", path="/recent-briefs")
    result = handler_module.handler(event, None)
